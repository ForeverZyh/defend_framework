...
class DataProcessor:
    def __init__(self, X, y, select_strategy=None, k=None, noise_strategy=None, **kwargs):
        ...
        if noise_strategy is not None:
            assert noise_strategy in ["feature_flipping", "label_flipping", "RAB_gaussian", "RAB_uniform"]
            if noise_strategy == "feature_flipping" or noise_strategy == "label_flipping":
                self.K = kwargs["K"]
                self.alpha = kwargs["alpha"]
                if noise_strategy == "feature_flipping":
    ...
    def process_train(self):
        ...
        if self.noise_strategy is not None:
            if self.noise_strategy == "feature_flipping":
            ...
            elif self.noise_strategy == "label_flipping":
            ...
            elif self.noise_strategy == "RAB_gaussian":
                ret_X += np.random.normal(0, self.sigma, ret_X.shape)
            elif self.noise_strategy == "RAB_uniform":
